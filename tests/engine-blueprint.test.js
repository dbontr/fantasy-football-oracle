...
test("model blueprint describes the complete analytics stack", () => {
  ...
  assert.equal(blueprint.layers.length, 16);
...
test("blueprint marks coaching and simulation as operational", () => {
  ...
  assert.ok(freeLearning.available.includes("final-season holdout gate"));
  assert.ok(freeLearning.available.includes("append-only forecast journal"));
});
